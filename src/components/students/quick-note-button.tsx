...
export function QuickNoteButton({ studentId, studentName, variant = "icon" }: Props) {
  ...
  async function save() {
    ...
    if (!description.trim()) {
      toast.error(t("writeNoteFirst"));
      return;
    }
    if (type === "follow_up" && !followUpDate) {
      toast.error(t("selectFollowUpDate"));
      return;
    }
    setSaving(true);
    try {
      ...
      toast.error(`${t("failedToSaveNote")}: ${msg}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("noteFor")} {studentName}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            ...
            {type === "follow_up" && (
              <div>
                <Label className="mb-1 block">{t("followUpDate")}</Label>
                <Input
                  type="date"
                  value={followUpDate}
            ...
            <div>
              <Label className="mb-1 block">{t("note")}</Label>
              <Textarea
                value={description}
